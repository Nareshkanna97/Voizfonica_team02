...
public class myPlansController {
   ...
   /* private List<Login> convertToDTOs(List<Order> models) {
    ...
    public String showUserPlans(Model model, @SessionAttribute("login") Login login, @ModelAttribute Order order) {
        ...
        List<PostpaidPlan> currentPost=postpaidRepo.findAllById(myPlans.get(0).getPostPaidId());
        List<PrepaidPlans> currentPre=prepaidRepo.findAllById(myPlans.get(0).getPrePaidId());
        List<DonglePlans> currentDongle=dongleRepo.findAllById(myPlans.get(0).getDongleId());
        model.addAttribute("currentPost",currentPost);
        model.addAttribute("currentPre",currentPre);
        model.addAttribute("currentDongle",currentDongle);
